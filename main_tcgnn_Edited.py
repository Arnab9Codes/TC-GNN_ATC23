...
# what is the number of classes in the cora dataset in test2
parser.add_argument("--classes", type=int, default=7, help="number of output classes")
parser.add_argument("--gdir", type=str, default='/mnt/huge_26TB/data/test2/', help="dataset location in mnt")
...
path = osp.join(args.gdir+dataset_name)

dataset = TCGNN_dataset(path, args.classes)
...
train_id = torch.tensor(dataset.train_id).cuda()
train_y_label =torch.tensor(dataset.train_y_label).cuda()
test_id = torch.tensor(dataset.test_id).cuda()
test_y_label = torch.tensor(dataset.test_y_label).cuda()
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        print(model)
        logp_train= model.forward()
        print('logp_train shape: ', logp_train.shape)
        print('logp_train: ', logp_train)
        print('logp_train shape:', logp_train.shape)
        #acc_val = pubmed_util.accuracy(logp_train, torch.tensor(train_y_label).cuda())
        acc_val = pubmed_util.accuracy(model()[test_id], test_y_label)
        
        print("testing accuracy: ", acc_val)
